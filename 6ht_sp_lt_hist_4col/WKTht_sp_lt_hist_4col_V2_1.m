...
%% plot
subplot(rows,cols, 1);numdis_se1 = mch_localtime_contourf(id_pr, localtime, mch);title('Jan 4');ylabel('Height (km)');
subplot(rows,cols, 2);numdis_se2 = mch_localtime_contourf(id_se, localtime, mch);title('Apr 4');
subplot(rows,cols, 3);numdis_se3 = mch_localtime_contourf(id_ap, localtime, mch);title('Jul 4');
subplot(rows,cols, 4);numdis_se4 = mch_localtime_contourf(id_ae, localtime, mch);title('Oct 2');
...
%% plot
subplot(rows,cols, 5);numdis_se5 = speed_localtime_contourf(id_pr, localtime, speed);ylabel('Speed (km/s)');
subplot(rows,cols, 6);numdis_se6 = speed_localtime_contourf(id_se, localtime, speed);
subplot(rows,cols, 7);numdis_se7 = speed_localtime_contourf(id_ap, localtime, speed);
subplot(rows,cols, 8);numdis_se8 = speed_localtime_contourf(id_ae, localtime, speed);
...
function numdis_se = speed_localtime_contourf(id_se, localtime, speed)
fs = 12; % fontsize
...
% % % % % % % % % % % % % % % % end
% % % % % % % % % % % % % % % % 
% % % % % % % % % % % % % % % % hold on;plot(0:1:24,speedpeak,'Color','k','linest','--','linewi',1);
...
hold on;plot(0:1:24,speedmed,'Color','k','LineWidth',2);
...
function numdis_se = mch_localtime_contourf(id_se, localtime, mch)
fs = 12; % fontsize
dashcolor = '#a5a5a5';
...
hold on;plot(0:1:24,mchpeak+mchwidth,'Color','k','linest','--','linewi',1);
hold on;plot(0:1:24,mchpeak,'Color','k','linewi',2);
hold on;plot(0:1:24,mchpeak-mchwidth,'Color','k','linest','--','linewi',1);
...
% % hold on; plot([maxmc maxmc],[70 170],'linest','--','color','k','linewi',1);
% % maxc=num2str(maxmc);
...
% hold on; plot([0 24],[htpeak_se htpeak_se],'linest','--','color','k','linewi',1);
% htadd_se=htpeak_se+htwidth_se;
% hold on; plot([0 24],[htadd_se htadd_se],'linest','--','color','k','linewi',1);
% htsub_se=htpeak_se-htwidth_se;
% hold on; plot([0 24],[htsub_se htsub_se],'linest','--','color','k','linewi',1);
...
maxmc=0+0.1*maxmc;
hold on; plot([maxmc maxmc],[0 ylim1],'linest','--','color','k','linewi',1);
maxmc=round(maxmc,2,"significant");
maxc=num2str(maxmc);
disp(['t_{peak}=' maxc ' hr']);
